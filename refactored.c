...
void server_add_client(server_ptr self, int fd) {
...
void server_accept(server_ptr self) {
	int fd;
	FD_CLR(self->fd, &self->to_write);
	if (!FD_ISSET(self->fd, &self->to_read))
		return;
	FD_CLR(self->fd, &self->to_read);
	fd = accept(self->fd, 0, 0);
	server_add_client(self, fd);
}
...
void main_server(main_type args) {
	server_ptr server = malloc(sizeof *server);
	if (!server)
		halt();
